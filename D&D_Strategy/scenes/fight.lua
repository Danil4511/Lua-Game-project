...
local function goSomewhere(event)
  ...
  --p.isVisible = true
  --pt.isVisible = true
  composer.hideOverlay( 'scenes.fight' )
  ...
  if (event=='Погибли') then
    ...
    newButtonM = widget.newButton {
      ...
      newButtonM = widget.newButton {
            ...
            fillColor = { default={249/255,243/255, 245/255, 1},over={150/255,0.3}},
            labelColor = {default={25/255,120/255,255/255}, over={1}},
            label = 'Карта',
...
local function roll()
  ...
  sc1 =   math.random(2, 6) +  math.random(2, 6)
...
local function chars(grp)
  -------------------------------
    ...
    textBox.text = "Механика этой игры предрасполагает к тому, что вы будете драться ( делать действие), бросая 2 кости из 6 граней. Ваш урон определяют именно они"
...
--CHARACTER ( HERO )
    ...
    --------NPS
    Orc = display.newImageRect(grp, 'img/orc_stone.png', 330, 350)
...
newButtonPick = widget.newButton {
      ...
      fillColor = { default={200/255,213/255, 225/255, 1},over={150/255,0.3}},
...
function scene:create( event )
    ...
    bg = display.newImageRect(sceneGroup, 'img/fight.jpg', 1100, 1920)
    bg.x = display.contentCenterX; bg.y = display.contentCenterY;
    ostSnd = audio.loadSound('snd/demonium.ogg', mainSndOptions)
