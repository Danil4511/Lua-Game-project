...
---TEXT
  ...
  gun.x, gun.y = display.contentCenterX, 1000
  gun:scale(1,-1)
  gun.Health = 100
 ...
 function crash(self, event)
  ...
  for i = 0, c do
    if (mas[i] == nil) then
		...
		local posx = {-100, 1200};
      local x = posx[math.random(1,2)]
      local y = math.random(-100, 1200)
...
spawnTimer = timer.performWithDelay(4000, spawn, 8)
--3 = 16
...
function spawnHP()
  ...
  HP.TIMER = timer.performWithDelay( 7000, remove )
end
...
function onUpdate()
  ...
  if sc == 35 then
    c = 10
    spawnTimer = timer.performWithDelay( 3000, spawn, 0 )
  end
...
function scene:create( event )
    ...
    bg = display.newImageRect(sceneGroup, 'img/easyBG2.jpg', 1500, 1920)
...
function scene:hide( event )
    ...
    elseif ( phase == "did" ) then
        ...
        print("play:hide: did")
        composer.removeScene('scenes.nPlay')
