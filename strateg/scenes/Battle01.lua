...
channel = 1,
  onComplete = function()
    audio.play(sndMas[math.random(1,3)])
  end,
}
...
local function goSomewhere()
  composer.removeScene( 'scenes.Battle01' )
  ...
  if res == 'Поражение!' then
    newButton = widget.newButton( {
      ...
      onPress = function()
        ...
        composer.hideOverlay( 'scenes.Battle01' )
  ...
  function movePers(event)
    ...
    if (hero2.x == mas[i-1].x and hero2.y == mas[i-1].y) then
      audio.play(heroAtt, {channel=2})
        ...
        ---
        mas[i-1].HP = mas[i-1].HP - math.random(1,4)
        mas[i-1].HPtext.text = mas[i-1].HP
        ----------------------
...
function spawn()
    ...
    mas[i] = display.newImageRect(grp,'img/enemy2.png', 135, 210)
...
--[[
    ...
    ---
    audio.play(heroAtt)
    event.other.HP = event.other.HP - math.random(3,4)
...
----------------------------------------
        ...
        ---
        hero.HP = hero.HP - math.random(2,5)
...
function scene:create(event)
    ...
    ost = audio.loadSound('snd/COMBAT03.mp3')
