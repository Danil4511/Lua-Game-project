...
local function goSomewhere()
  composer.removeScene( 'scenes.Battle21' )
  ...
  if res == 'Поражение!' then
    newButton = widget.newButton( {
      ...
      onPress = function()
        ...
        composer.hideOverlay( 'scenes.Battle21' )
  ...
  function movePers(event)
    ...
    if (hero2.x == mas[i-1].x and hero2.y == mas[i-1].y) then
      audio.play(heroAtt, {channel=2})
        ...
        ---
        mas[i-1].HP = mas[i-1].HP - math.random(2,4)
        mas[i-1].HPtext.text = mas[i-1].HP
        ----------------------
      ...
      elseif (hero.x == mas[i-2].x and hero.y == mas[i-2].y) then
        ...
        -------------------
          ...
          ---
          mas[i-2].HP = mas[i-2].HP - math.random(2,4)
          mas[i-2].HPtext.text = mas[i-2].HP
          ----------------------
    end
...
local jj = 1
local enm = {'img/enemy.png','img/enemy2.png'}
...
----------------------------------------
        ...
        ---
        hero.HP = hero.HP - math.random(1,3)
        ...
        ----------------------------------------
                ...
                ---
                hero2.HP = hero2.HP - math.random(1,4)
