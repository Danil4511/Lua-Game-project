...
local function goSomewhere(event)
  ...
  composer.removeScene('scenes.nPlay3', false)
...
local function chars(grp)
  ...
  local spider_info2 = require('img.spiders')
  local spider_sheet2 = graphics.newImageSheet('img/spiders.png', spider_info2:getSheet());
...
spawnTimer = timer.performWithDelay(3000, spawn, 5)
--3 = 16
...
function spawn2()
 ...
 local function movePers(event)
     for i= 0,c2 do
     transition.moveTo(mas2[i], {x = gun.x, y = gun.y, time=3400})
     end
 end
...
function crash(self, event)
 ...
 for i = 0, c2 do
   if (mas2[i] == nil) then
     ...
     local y = math.random(-100, 0)
     --mas[i] = display.newCircle(x, y,50)
     mas2[i] = display.newSprite(spider_sheet2, spider_data2);
     mas2[i].x = x; mas2[i].y = y;
     mas2[i]:setSequence('attack')
     mas2[i]:play();

     --mas[i]:setFillColor(0, 120/255, 200/255)
     mas2[i].ID = 'ENEMY'

     physics.addBody(mas2[i], 'dinamic')
     mas2[i].isALIVE = true
     mas2[i].gravityScale = 0
     mas2[i].isSensor = true
     mas2[i].Timer = timer.performWithDelay( 500, movePers )
     mas2[i].collision = crash
     mas2[i]:addEventListener('collision', mas2[i])
...
spawnTimer2 = timer.performWithDelay(3000, spawn2, 5)
...
function onUpdate()
  ...
  if sc == 30 then
    ...
    spawnTimer = timer.performWithDelay( 5000, spawn2, 0 )
  end
...
function scene:create( event )
    ...
    bg = display.newImageRect(sceneGroup, 'img/hero33.jpg', 1500, 1920)
    bg.x = display.contentCenterX; bg.y = display.contentCenterY;
    ostSnd = audio.loadSound('snd/mus/Blackmoor Colossus.mp3', mainSndOptions)
    ...
    spSnd2 = audio.loadSound('snd/spDeath.wav', adSndOptions)
...
function scene:hide( event )
    ...
    elseif ( phase == "did" ) then
        ...
        print("play:hide: did")
        composer.removeScene('scenes.nPlay3')
